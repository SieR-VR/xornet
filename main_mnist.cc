...
int main()
{
    Dense<784, 1000> dense1 = Dense<784, 1000>();
    Dense<1000, 1000> dense2 = Dense<1000, 1000>();
    Dense<1000, 1000> dense3 = Dense<1000, 1000>();
    Dense<1000, 10> dense4 = Dense<1000, 10>();
    ...
    for (int i = 0; i < test_size; i++) {
        auto output = dense4.forward(dense3.forward(dense2.forward(dense1.forward(train_images[i]))));
        correct += (output == test_labels[i]);
    }

    std::cout << std::endl << "Accuracy: " << correct / (float)test_size * 100.f << "%" << std::endl;
    std::cout << correct;
}